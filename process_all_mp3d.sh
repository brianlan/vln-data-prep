...
# Process every locally downloaded MP3D Zed trajectory. Git-LFS pointer files
# are skipped so this script never triggers an accidental bulk fetch.

SCRIPT_DIR=/home/rlan/projects/vln-data-prep
TRAJ_DIR=/ssd5/datasets/InternData-N1/vln_n1/traj_data/matterport3d_zed
LOG_DIR=/tmp/opencode/mp3d_batch_logs
...
while IFS= read -r archive; do
    ...
    if bash "${SCRIPT_DIR}/run_pipeline_mp3d.sh" "$scene" > "${LOG_DIR}/${scene}.log" 2>&1; then
