...
"""Render equidistant fisheye RGB and depth from textured HM3D GLBs.

HM3D distributes one self-contained textured GLB per scene. Although glTF is
nominally Y-up, Habitat's HM3D GLBs contain coordinates already aligned with
the Z-up InternData world. Blender applies the standard glTF Y-up conversion
on import, so this script immediately applies its inverse to the imported root
objects. InternData action matrices can then be used directly as cam-to-world.
"""
...
def import_textured_glb(filepath):
    """Import an HM3D GLB, undo Blender's axis conversion, and return meshes."""
    import bpy
    ...
    # Blender's glTF importer maps (x,y,z)->(x,-z,y). HM3D's Habitat-ready GLBs
    # and InternData actions already agree in their stored Z-up coordinates, so
    # apply the inverse mapping (x,y,z)->(x,z,-y) to each imported root.
    undo_gltf_axis_conversion = Matrix.Rotation(np.radians(-90.0), 4, "X")
    roots = [obj for obj in imported if obj.parent not in imported_set]
    for obj in roots:
        obj.matrix_world = undo_gltf_axis_conversion @ obj.matrix_world
    print(f"[render] Undid glTF axis conversion on {len(roots)} imported root object(s)")

    return [obj for obj in imported if obj.type == "MESH"]


def make_materials_emissive(mesh_objects):
    """Render captured HM3D base colors without applying artificial lighting.

    HM3D is a reconstructed real scan whose diffuse textures already contain
    the captured appearance. The existing base-color node graph is connected
    to an emission shader, preserving texture/color factors while avoiding a
    ...
    for mat in materials:
        ...
        emission.name = "HM3D Captured Appearance"
    ...
    if not materials:
        raise RuntimeError("HM3D GLB imported without any materials")
    if textured == 0:
        raise RuntimeError(
            "HM3D materials loaded, but no base-color texture was found. "
            "Check that the downloaded GLB is the textured HM3D asset."
        )
...
def validate_pose_bounds(actions_by_episode, bounds_min, bounds_max, margin=0.5):
    """Fail early when the mesh and trajectory coordinate frames disagree."""
    ...
    if outside.any():
        ...
        raise RuntimeError(
            f"{outside.sum()}/{len(positions)} camera positions are outside HM3D bounds; "
...
def parse_args():
    ...
    parser.add_argument("--glb", required=True, help="Path to the textured HM3D scene GLB")
...
def main():
    ...
    print(
        f"[render] Scene={args.scene}, GLB={args.glb}, FOV={args.fov_deg}deg, "
    ...
    print(f"[render] Importing textured HM3D GLB: {args.glb}")
    mesh_objects = import_textured_glb(args.glb)
    if not mesh_objects:
